...
## Consent and non-egress, inherited, not reimplemented

Two invariants this connector does not implement itself, because the pipeline
already enforces them upstream of every connector:

* **Consent.** `pipeline.export` partitions golden records by consent
  (`consent.py`) before any connector is constructed. Under a consent-requiring
  policy pack (`dv`, `hipaa`), a record without granted consent never reaches
  this connector's `write_all`; it is written to `withheld.csv` instead, by id
  and reason only.
* **Non-local egress.** `WebhookConnector.is_local = False`, the same as
  CiviCRM and Salesforce. Under the `dv` policy pack,
  `pipeline.build_connector` refuses to construct this connector at all,
  fail-closed, before any client data is touched:
  ...
  under that pack instead.

There is no separate "is this a new inbound data path, does it need its own
consent check" question to answer here: the connector sits at the same choke
point in the pipeline every write destination sits at, and inherits both
gates by construction. See `tests/test_no_egress.py` and
`tests/test_connectors_webhook.py` for the enforcing tests.
...
* `fields` carries only the canonical fields the recipe's `[mapping]` section
  activated and that resolved to a non-empty value for this record — the same
  "no field this run did not touch" rule the CSV and CRM connectors follow.
  Canonical field names are listed in `models.CANONICAL_FIELDS`:
...
takes toward misconfiguration (an unknown recipe key, an unknown policy pack)
rather than sending a partial run's records to nowhere and discovering the
mistake in a support ticket.
...
* **No inbound receiver.** This module only sends. It does not open a
  ...
  pipeline's only intake path today is `[input] incoming`/`existing`
  (CSV, or a CSV/PDF-mixed folder — see `pipeline._ingest_source`); this
  project has no pluggable-source concept parallel to `connectors/` yet, so
  "accept a webhook as a data source" is a different, larger feature
  (a new ingestion path, not this connector) and is out of scope here.
