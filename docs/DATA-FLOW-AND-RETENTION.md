...
| `ai_ocr_proposals.json` | `assistant/ocr_propose.py` via `reconcile ai-propose-corrections` | the `--out` directory | Yes: raw field values, proposed values, and verbatim quoted source text | Written by `reconcile ai-propose-corrections` only. In the destruction inventory (`destruction.PII_ARTIFACTS`). |
