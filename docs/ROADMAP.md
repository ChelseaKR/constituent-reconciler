...
| Extraction field precision and recall | At least 0.95 precision and 0.90 recall on the committed labeled fixture, for each document type on its own; observed 1.00 precision and 0.941 recall on the PDF text layer, and 1.00 precision and 0.960 recall on photographed and scanned page images read by the real Tesseract binary, in `eval/extraction-report.md` | REVIEW | `eval/extraction-report.md`, regenerated by `make eval-extraction`, drift-gated in CI | maintainer |
