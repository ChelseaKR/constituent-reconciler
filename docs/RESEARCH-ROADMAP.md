...
| R3 | **Ship the supply-chain hardening**: SBOM, Sigstore-signed releases, SHA-pinned actions, OIDC, secret scanning. ✅ Implemented 2026-07-02 (release workflow with Sigstore over OIDC, `make sbom`/`make build`, Dependabot; secret scanning and push protection enabled in repo settings) | C1, F1, E1 | P1 | M | README Standards; `DPG-CONFORMANCE.md` indicator 8 "partially landed"; metrics ledger; `RESPONSIBLE-TECH-AUDITS.md` security. **[corroborates ROADMAP / DPG note]** |
