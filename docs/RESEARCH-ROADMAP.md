...
| R5 | **Measure and report bias by name class** (transliterated, hyphenated, non-Western order) **and address class** (rural/informal) on the eval fixtures, with mitigations. ✅ Implemented 2026-07-02 (roadmap branch, unmerged): per-class fixtures + `per_class_metrics`, "Bias by class" in `eval/report.md` | C2, F1, D2, A2 | P1 | M | `RESPONSIBLE-TECH-AUDITS.md` bias TODO; `DPG-CONFORMANCE.md` indicator 9 residual risk. **[corroborates audits / DPG note]** |
