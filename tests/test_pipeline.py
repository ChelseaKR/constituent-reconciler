...
def test_export_via_webhook_creates_and_logs_provenance(tmp_path: Path) -> None:
    ...
    # Default policy exports all 21 resolved records; the webhook connector reports
    # each as a plain "written" (no upsert-lookup response to read created/updated from).
...
def test_export_via_webhook_withholds_non_consented_records_before_any_post(
    tmp_path: Path,
) -> None:
    # hipaa requires consent but, unlike dv, does not require a local write target,
    # so it is the pack that isolates "consent gate" behavior from "non-local
    # refusal" behavior for this test: the webhook connector is allowed to build,
    # but a withheld record must never reach it.
    recipe = replace(
        load_recipe(EXAMPLES / "recipe.toml", policy_pack="hipaa"),
        output=OutputConfig(connector="webhook", endpoint="https://example.org/hooks/reconciler"),
    )
    result = pipeline.run(recipe)
    transport = _WebhookTransport()
    ...
    assert "N009" not in sent_external_ids
    # No record's field values (name, email, etc.) leaked into the withheld artifact.
    for record in result.golden:
        if "N009" in record.members:
            for value in record.fields.values():
                if value:
                    assert value not in withheld_text
