...
- `identity`: 250 of the 9997 records with a name (10000 in all), sampled under seed 20260906 and capped at 250; 3 records with no value in `first_name` or `last_name` were excluded before sampling, because a twin of a record that carries no name is not the easiest possible case.
