...
def name_pair_key(first_normalized: str, last_normalized: str) -> str:
    """Return an order-free key over a record's two name values.

    The two normalized names are sorted and joined, so a record filed as
    given="Wei" family="Li" and one filed as given="Li" family="Wei" produce
    the same key. Used as a blocking key (``defaults.blocking_rules_for``) so
    that a transposed-name duplicate is at least offered to the scorer; the
    scorer then decides on the transposition comparison level, which is
    tolerant of a typo the key is not.

    Transposition is worth a key of its own because it is a common intake
    error and a structural one, not a typo. A paper or web form labels one box
    "first name" and the other "last name", and the ordering that assumes is
    not universal: family-name-first is the written convention in Chinese,
    Korean, Japanese, Hungarian and Vietnamese naming, so a constituent
    writing their name the way they always write it lands the two values in
    the opposite boxes from the intake worker who typed the earlier record.
    Every other blocking rule reads one field at a time and cannot see across
    the two, so without this key such a pair is only ever generated by
    coincidence on date of birth.

    Returns ``""`` unless both values are present, matching every other
    derived key in this module: a key built from one name would collide with
    unrelated records rather than carry evidence.
    """

    if not first_normalized or not last_normalized:
        return ""
    return "|".join(sorted((first_normalized, last_normalized)))
...
def normalized_keys(fields: tuple[str, ...]) -> frozenset[str]:
    ...
    key for ``first_name``, the phonetic key and surname tokens for
    ``last_name``, and the order-free ``name_pair_key`` when both name fields
    are active. stage_cache.py validates cached payloads against this set, so
    keep it in step with what ``normalize_record`` writes.
    """
...
def normalize_record(
    ...
    *,
    ...
    (``defaults.py``): a nickname-group key for ``first_name``, a phonetic key
    plus two surname tokens for ``last_name``, and an order-free
    ``name_pair_key`` over the two together. These are stored under their own
    keys in ``normalized`` rather than as separate canonical fields, since they
    only ever exist as a function of the base fields and a recipe never maps
    them directly. ``name_pair_key`` is filled after the per-field loop because
    it is the one derived column that reads two canonical fields rather than
    one.
    ...
    if "first_name" in fields and "last_name" in fields:
        normalized["name_pair_key"] = name_pair_key(
