...
def _calibration_lines(
    calibration: CalibrationReport | None, *, field_judge_ran: bool = True
) -> list[str]:
    if not field_judge_ran:
        return [
            "",
            "## Calibration (LLM field judge)",
            "",
            "Not applicable to this run. The LLM field judge never executed: the "
            "corpus is structured CSV, so no extraction seam was invoked and "
            "there are no confidence verdicts for human labels to agree with. "
            "This is not a waived gate. Reporting a kappa failure for a "
            "component that did not run would be a false claim about coverage, "
            "the same way a fixed provenance sentence was; the gate itself still "
            "fails closed wherever the judge does run.",
        ]
    lines = [
        "",
        "## Calibration (LLM field judge)",
...
def render_eval_markdown(
    ...
    *,
    ...
    personal data -- printed unconditionally, so pointing `constituent-reconcile eval` at a
    ...
    Callers now pass what they know. `constituent-reconcile eval` reads an explicit
    ...
    ``generator`` names the command that produced the report. It used to be
    hard-coded to ``reconcile eval``, so the reports written by the corpus and
    benchmark runners credited a command that had not been run and that would not
    ...
    lines = [
        "# Eval report",
        "",
        f"Dataset: `{dataset}`. Generated by `{generator}`. This file is "
        ...
        "",
        "F1 is reported for comparability with published record-linkage results "
        "and is not a gate. It weighs a false merge and a missed match equally, "
        "which this pipeline does not.",
    ]
    if report.segments:
        lines += [
            "",
...
def render_extraction_markdown(
    ...
    *,
    ...
    lines = [
        ...
        "",
        f"Fixture set: `{dataset}`. Generated by `constituent-reconcile eval-extraction`. "
