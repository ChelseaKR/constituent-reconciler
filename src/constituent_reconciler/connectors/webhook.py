...
* **Consent.** ``pipeline.export`` runs the consent gate
  (``consent.partition_by_consent``) before any connector is built or
  touched, passing ``destination=recipe.output.connector`` (``"webhook"``
  for this connector). A golden record whose consent is not active for that
  destination -- absent, revoked, expired, future-dated, or scoped to other
  connectors only -- never reaches ``write_all``. This connector adds no
  consent logic of its own; it inherits the same lifecycle-aware gate every
  other connector inherits, scope included.
